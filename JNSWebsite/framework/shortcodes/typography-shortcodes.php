...
function mo_blockquote_shortcode($atts, $content = null, $shortcode_name = "") {
    ...
    $output = '<blockquote ' . $id . ' class="align' . $align . $class . '"' . $style . '>' . do_shortcode($content) . $author_info . '</blockquote>';
...
function mo_heading2_shortcode($atts) {
    ...
    $separator = mo_to_boolean($separator) ? ' separator' : '';

    if (!empty($style))
        $style = ' style="' . $style . '"';
    if (!empty($class))
        $class = ' ' . $class;
    $output = '<div class="heading2' . $separator . $class . '"' . $style . '>';
...
class - Custom CSS class name to be set for the div element created (optional)
title - A string value indicating the title of the heading.
separator - boolean (true) Specify if a separator needs to be shown.


*/

function mo_heading1_shortcode($atts) {
    extract(shortcode_atts(array(
            'style' => '',
            'class' => '',
            'title' => '',
    ...
    if (!empty($class))
        $class = ' ' . $class;
    $output = '<div class="heading1' . $separator . $class . '"' . $style . '>';
...
add_shortcode('heading1', 'mo_heading1_shortcode');
...
This shortcode is used to create a fancy link to a URL. The link displays with an arrow on the right which responds to mouse hover.

The content of the shortcode has the link text.

Usage:

[fancy_link href="http://mydomain.com/link" title="My Link"]My Link[/fancy_link]

Parameters -

href - The URL to which the link points to.
title - The

*/

function mo_fancy_link_shortcode($atts, $content = null, $shortcode_name = "") {
    ...
    return '<a class="fancy-link" href="'. $href . '" title="'. $title. '">' . do_shortcode($content) . '&nbsp;&nbsp;<i class="icon-arrow-right3"></i></a>';
}

add_shortcode('fancy_link', 'mo_fancy_link_shortcode');